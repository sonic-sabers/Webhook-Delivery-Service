...
export interface DeliveryAttempt {
  ...
  attempt_number: number;   // 0 = queued, not yet attempted; incremented to 1 on first retry schedule
  max_attempts: number;
  next_attempt_at: number;
  claimed_at: number | null;  // set when worker claims row; sweeper uses this for stale detection
...
export function createAttempt(
  ...
  getDb().prepare(`
    ...
    VALUES (?, ?, ?, 'pending', 0, ?, ?, ?)
  `).run(id, eventId, subscriptionId, maxAttempts, now, now);
}
...
// Atomic claim: transaction ensures no two concurrent async calls can claim the same rows.
// Returns rows that were successfully claimed.
...
// claimed_at is set at delivery start; created_at is attempt birth — wrong for stale detection.
...
// Crash mid-fanout can't leave an event without attempts.
